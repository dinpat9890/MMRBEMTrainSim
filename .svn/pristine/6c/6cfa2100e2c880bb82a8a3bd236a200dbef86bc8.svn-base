...
/**
 * This class handles ECN Component Switch C Status update from vehicle outputs
 * 
 */
public class ECNComponentSwitchCStatusVehicleOutputsRequestHandler implements
    ITCMSVehicleOutputsRequestHandler<TCMSOutputsECNScreenImpl<Integer, IAppOutputsECNScreenHandler>>
{

  protected final List<Entry<VehicleECNComponentSwitchStatusDO>> componentSwitchCStatus = new ArrayList<>();
  ...
  public void handleRequest(TCMSOutputsECNScreenImpl<Integer, IAppOutputsECNScreenHandler> request)
  {
    try
    {
      VehicleECNComponentSwitchStatusDO vehicleECNComponentSwitchStatusDO = ITCMSVehicleOutputsRequestHandler
          .getVehicleStatusDo(componentSwitchCStatus, request.getCarIndex(),
      ...
      ITCMSVehicleOutputsRequestHandler.updateVehicleStatusDOList(componentSwitchCStatus,
          vehicleECNComponentSwitchStatusDO, request.getCarIndex());
      setComponentSwitchStatus(componentSwitchCStatus, request.getVehicleDetailModelList(),
          builder -> request.getHandler().setEcnCsCStatusList(builder.build().toByteArray()));
    }
      ...
      VehicleECNComponentSwitchStatusDO vehicleECNComponentSwitchStatusDO)
  {
    ...
    {
      vehicleECNComponentSwitchStatusDO.setComponentSwitchCFault(newvalue);
    }
    else if (propertyKey.equals(ITCMSVehicleOutputsECNScreenProperties.CONSIST_SWITCH_STATUS_C))
    {
      vehicleECNComponentSwitchStatusDO.setComponentSwitchCStatus(newvalue);
    }
  }
  ...
  private ECNComponentSwitchState getComponentSwitchState(VehicleECNComponentSwitchStatusDO data)
  {
    ...
    if (data.getComponentSwitchCFault() == TCMSConstants.ECN_COMPONENT_FAULTY)
    {
      ecnComponentSwitchState = ECNComponentSwitchState.CS_FAULTY;
    }
    else if (data.getComponentSwitchCStatus() == TCMSConstants.ECN_COMPONENT_OK)
    {
