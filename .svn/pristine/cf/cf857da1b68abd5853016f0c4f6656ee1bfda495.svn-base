...
/**
 * Status screens have table to show the data, this helper class update the
 * table data list to pass on its respective controller class.
 * 
 */
public class TableDataProcesser
{
  ...
  private void createData(ITableRowProperty rowProperty, int maxCars)
  {
    ...
    if (create)
    {
      ...
      for (int i = 0; i < maxCars; i++)
      {
        ...
        tableCellDo.setFirstBogieCellState(DMIConstants.DOES_NOT_EXIST_STATE);
        tableCellDo.setSecondBogieCellState(DMIConstants.DOES_NOT_EXIST_STATE);
        tableCellDo.setSecondBogieCellDataExist(false);
  ...
  public void updateData(ITableRowProperty rowProperty, int carIndex, boolean isSecondBogieExist,
      String firstBogieCellData, String firstBogieCellState)
  {
    updateData(rowProperty, carIndex, isSecondBogieExist, firstBogieCellData, firstBogieCellState, "", //$NON-NLS-1$
        DMIUIConstants.NOT_APPLICABLE_CSS);
  }
  ...
  public synchronized void emptyData(ITableRowProperty rowProperty, int maxCars)
  {
   ...
   *
   * @param carIndexList
   *          the list of currently available all car index. Other than these
   *          car index all other car data will be removed
   */
