...
public class HVACCommandDMRearCarController extends AbstractTCMSController
{
  ...
  public HVACCommandDMRearCarController(CarConfigurationModel carConfig,
  ...
  void isolateHvac1BtnAction(ActionEvent event)
  {
        ...
        HVACOperatingState.HVAC_OPERATING_ISOLATED.toString()))
    {
      ...
      TCMSFactory.getServiceRequestHandler(scenarioCarIndex).sendRequest(VDUInputs.ISOLATE_HVAC1_CAR7_REQ,
  ...
  void isolateHvac2BtnAction(ActionEvent event)
  {
    ...
    if (Objects.equals(havac2State.get().getComponentState(),
        HVACOperatingState.HVAC_OPERATING_ISOLATED.toString()))
    {
      TCMSFactory.getServiceRequestHandler(scenarioCarIndex).sendRequest(VDUInputs.ISOLATE_HVAC2_CAR7_REQ,
          TCMSUIConstants.REINSERT_VALUE);
    }
    else
    {
