...
#ifndef OBSTRUCTION_SHEEP_US_CPP
#define OBSTRUCTION_SHEEP_US_CPP

#include <AnimatedFeature.us.cpp>

namespace Features
{
    /// Sheep obstruction feature
    class ObstructionSheep : AnimatedFeature
    {
    private:
        int m_valid;
        enum
    ...
    public:
        ...
        /// Constructs a new instance of a ObstructionSheep feature. The constructor
        ...
        ObstructionSheep(FeatureInfoExtern info, Properties properties, Node parent)
        {
            __AnimatedFeature__(info, properties, parent);

            // Attempt to load the sheep node. If we are successful loading it, register
            // with the AnimatedFeature class and then associate each state with its set of actions.
            Node sheep                      = engine.world.loadNode("sheep.node");
            ObjectMeshSkinned skinned_sheep = (sheep == 0) ? 0 : node_cast(sheep);
            if(skinned_sheep != 0)
            {
                registerNode(skinned_sheep);

                int sheep_idle_animation_id   = -1; // Sheep Idle
                int sheep_walk_animation_id   = -1; // Sheep Walk

                sheep_idle_animation_id = skinned_sheep.addAnimation("sheep_idle.anim");
                sheep_walk_animation_id = skinned_sheep.addAnimation("sheep_walk.anim");


                // Now define the state transitions 1 - Not Visible 2 - Off Track 3 - On Track.
                setAction(NOT_VISIBLE, new NodeEnable(skinned_sheep, 0));
                setAction(OFF_TRACK, new NodeActionSequence((new ObstructionMoveAndAnimate(skinned_sheep, sheep_walk_animation_id, 45.0, 2.0, 180.0, translate(dvec3(-5.0, 0.0, 0.0)) * rotateZ(180.0)), new NodeActionLoop(new ObstructionNodeAnimate(skinned_sheep, sheep_idle_animation_id, 15.0)))));
                setAction(ON_TRACK, new NodeActionSequence((new ObstructionMoveAndAnimate(skinned_sheep, sheep_walk_animation_id, 45.0, 2.0, 180.0, translate(dvec3(0.0, 0.0, 0.0)) * rotateZ(180.0)), new NodeActionLoop(new ObstructionNodeAnimate(skinned_sheep, sheep_idle_animation_id, 15.0)))));
                m_valid = 1;
            }
            else
            {
                log.error("Unable to load sheep obstruction - invalid node file for feature with id %d\n", info.getID());
                m_valid = 0;
            }
        }
        ...
        /// This class allows us to move and animate obstruction (in this case it is sheep). This a specific case of
        ...
        /// This class represents the action to animate the given obstruction node sheep (which is
    ...
    /// feature type (ObstructionSheep).
    class ObstructionSheepType : FeatureType
    {
    public:
        ...
        string getName()
        {
            return "ObstructionSheep";
        }

        /// Factory function - creates a ObstructionSheep using FeatureInfo and Properties.
        ///
        ...
        {
            return new ObstructionSheep(info, properties, parent);
        }
    };
}
