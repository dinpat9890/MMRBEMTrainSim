 ...
 * ==================================================================
 */
package com.sydac.mmrbem.radio.fe.base;
...
import com.sydac.mmrbem.radio.fe.app.RadioApplication;
import com.sydac.mmrbem.radio.fe.base.impl.FxFrontendSceneManagerImpl;
import com.sydac.mmrbem.radio.fe.base.utils.TimeUtil;
import com.sydac.mmrbem.radio.fe.comms.CommsBinding;
import com.sydac.mmrbem.radio.fe.comms.CommsBindingImpl;
...
/**
 * TCMS Frontend Panel Manager that bridges the simulator session with the
 * JavaFx application's creation and presentation
 *
 * @author Nazim Oliyath
 */
public class RadioFrontendPanelManager extends AbstractFrontendPanelManager
{
  ...
  public RadioFrontendPanelManager(PeerApplication baseApplication)
  {
      ...
      int carIndex)
  {
    ...
    RadioFactory.register(carIndex, hubObjectProvider);
    RadioFactory.register(carIndex, new CommsBindingImpl(aGateway, carIndex));
  }
      ...
      BaseSessionData aSessionData, boolean aIsSimulation, IHubObjectCallback aCallback)
  {
    ...
    Map<Integer, CommsBinding> commsBindingMap = RadioFactory.getCommsBinding();
    if (aCallback == null) // GMI Plugin execution path
    {
      for (Entry<Integer, CommsBinding> commsBindingEntry : commsBindingMap.entrySet())
      {
        ...
        screenChangeListener.attachToModel(RadioFactory.getCommsBinding(carIndex));
    ...
    // FrontendModule execution path
    {
      ...
      {
        int carIndex = commsBindingEntry.getKey();
        callbacks.put(carIndex, new CompositeHubObjectCallback(carIndex, commsBindingEntry.getValue(),
    ...
    if (aIsSimulation)
    {
      monitor = new IODataObjectCreator(aGateway, aApplicationName, logicalHubId, creatorId,
          ProjectObjectConstants.ClassId.RADIO, scenarioTrainId, callbacks,
          ProjectObjectDefinition.getObjectDefinition(ProjectObjectConstants.ClassId.RADIO_INPUTS),
          ProjectObjectDefinition.getObjectDefinition(ProjectObjectConstants.ClassId.RADIO_OUTPUTS),
          ProjectObjectDefinition.getObjectDefinition(ProjectObjectConstants.ClassId.RADIO_INPUTS_VALUES));

    }
    else
    {
      monitor = new IODataObjectMonitor(aGateway, logicalHubId, creatorId,
          ProjectObjectConstants.ClassId.RADIO, scenarioTrainId, callbacks);
    }
  ...
  public void createFxApplication(int carIndex, Properties properties)
  {
    ...
    RadioApplication.startApplication(xpos, ypos, width, height, sceneManager);
  }
  ...
  public void showFxApplication(final int carIndex, final boolean show)
  {
    if (show)
    {
      RadioApplication.showApplication(carIndex, dbScenarioTrain);
    }
    else
    {
      RadioApplication.hideApplication();
    }
  }
  ...
  {
    RadioApplication.pauseApplication(pause);
  }
   ...
   * @author Nazim Oliyath
   */
  public static final class CompositeHubObjectCallback implements IHubObjectCallback
  {
    ...
    public CompositeHubObjectCallback(int carIndex, IHubObjectCallback hubObjectCallback,
  ...
  protected static final class FxHubObjectCallback implements IHubObjectCallback
  {
    ...
    {
      hubObjectCallback.onHubInputCreation(hubInput);
      CommsListenerManager.attachToModel(carIndex, RadioFactory.getCommsBinding(carIndex));
    }
