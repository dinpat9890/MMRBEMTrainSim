...
public class RingProgressIndicatorSkin implements Skin<RingProgressIndicator>
{
  ...
  public RingProgressIndicatorSkin(final RingProgressIndicator indicator)
  {
    ...
    this.indicator.progressProperty().addListener((o, oldVal, newVal) -> {
      ...
      {
        setProgressLabel(showNumber(newVal.intValue()));
    ...
    container.getChildren().addAll(fillerArc, outerCircle, percentLabel);
  }
  ...
  {
    setProgressLabel(showNumber(value));
  ...
  public int showNumber(int val)
  {
    int displayNumber = 0;
    if (val > 0)
    {
      displayNumber = val / 10;
