...
///
/// @copyright  (C) Copyright:  Common Pty Ltd, 2020
...
const std::string UNIT_PROPERTIES_CONFIG_SECTION_NAME       = "Unit Properties";
const std::string PIS_INTERCOM_UNIT_PLUGIN_NAME             = "PISIntercomUnit";
const std::string DEFAULT_INTERCOM_UNIT_CONFIG_FILE_NAME    = "PISIntercom.ini";

// == GLOBAL VARIABLES =============================================================================
Common::Properties *g_properties;
Common::Properties *g_pis_intercom_properties;
...
UNIT_DLL_API Comms::UnitCreators GetParticipatingUnitCreators(Common::Resources &resources, Comms::Gateway &gateway)
{
    ...
    if (!gateway.IsReplay() && !session_data.preview)
    {
        ...
        auto &data_access = resources.Get<Common::Data::DataAccess>();

        // Get the IDs of the simulated trains in the scenario
        ...
        {
            //// Need to determine car class at this point!
            Train::Database::Train train_data_access(data_access);

            const auto scenario_train = train_data_access.GetScenarioTrain(scenario_data.scenario_id, scenario_train_id);

            if (scenario_train.is_simulated)
            {
                std::string pis_intercom_property_file_name = g_properties->GetProperty(UNIT_PROPERTIES_CONFIG_SECTION_NAME,
                                                                                        PIS_INTERCOM_UNIT_PLUGIN_NAME,
                                                                                        DEFAULT_INTERCOM_UNIT_CONFIG_FILE_NAME);

                g_pis_intercom_properties = new Common::Properties(pis_intercom_property_file_name);
                auto scenario_train_id = scenario_train.scenario_train_id;

                unit_creators.push_back([&, scenario_train_id]()
                {
                    return std::make_shared<Intercom::PISIntercomUnit>(gateway,
                                                                       *g_properties,
                                                                       *g_pis_intercom_properties,
                                                                       scenario_train_id);
                });
            }
