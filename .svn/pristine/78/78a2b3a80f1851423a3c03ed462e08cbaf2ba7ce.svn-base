...
import com.sydac.mmrbem.tcms.fe.gui.binder.PISSystemTestScreenBinder;
...
public class PISSystemTestScreenController extends AbstractTCMSController
{

  @FXML
  private Button pisTestUpButton;

  @FXML
  private Button pisTestDownButtoon;

  @FXML
  private Button pisTestStartButton;

  @FXML
  private Label pisTestScreenLabel;

  @FXML
  private TableView<TestScreenTableDO> pisTestScreenTable;
  ...
  public void initializeImpl()
  {
    ...
    tcmsScreenBinders.add(new PISSystemTestScreenBinder(testSystemScreenModel));

    currentPageListener = (observable, oldVal, newVal) -> updatePISSystemTestTable();
    testSystemScreenModel.getCurrentPageNumber().addListener(currentPageListener);

    totalPagesListener = (observable, oldVal, newVal) -> updatePISSystemTestTable();
    testSystemScreenModel.getTotalPageNumber().addListener(totalPagesListener);

    updatePISSystemTestTable();
  ...
  private void setTableProperties(ListProperty<TestScreenTableDO> eventList)
  {
    ...
    pisTestScreenTable.setFixedCellSize(CELL_HEIGHT * 1.0);

    pisTestScreenTable.getSelectionModel().setCellSelectionEnabled(false);
    pisTestScreenTable.getSelectionModel().clearAndSelect(0);
    pisTestScreenTable.setPadding(new Insets(0, 0, 0, 0));

    pisTestScreenTable.widthProperty().addListener((source, oldWidth, newWidth) -> {
      Pane header = (Pane)pisTestScreenTable.lookup("TableHeaderRow"); //$NON-NLS-1$
      if (header.isVisible())
      {
    ...
    pisTestScreenTable.getColumns().add(getColumn("carCode", COMPONENT_WIDTH * 1.0, "CAR")); //$NON-NLS-1$ //$NON-NLS-2$
    pisTestScreenTable.getColumns().add(getColumn("componentName", TEST_STATUS_WIDTH * 1.0, "COMPONENT")); //$NON-NLS-1$ //$NON-NLS-2$
    pisTestScreenTable.getColumns().add(getColumn("testStatus", LAST_UPDATE_WIDTH * 1.0, "TEST STATUS")); //$NON-NLS-1$ //$NON-NLS-2$
    pisTestScreenTable.getColumns().add(getColumn("lastUpdateDate", LAST_UPDATE_WIDTH * 1.0, "LAST UPDATE")); //$NON-NLS-1$ //$NON-NLS-2$

    pisTestScreenTable.setColumnResizePolicy(TableView.CONSTRAINED_RESIZE_POLICY);

    pisTestScreenTable.setItems(eventList);

    pisTestScreenTable.getSelectionModel().selectedIndexProperty()
        .addListener(new RowChangeHandler(pisTestScreenTable, eventList));
  ...
  private void updatePISSystemTestTable()
  {
    pisTestScreenLabel.setText("Showing page " + TCMSUIConstants.CURRENT_PAGE + " of " //$NON-NLS-1$ //$NON-NLS-2$
