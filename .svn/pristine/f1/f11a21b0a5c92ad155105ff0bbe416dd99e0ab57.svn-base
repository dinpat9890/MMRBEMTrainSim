...
import com.sydac.mmrbem.dmi.fe.base.utils.TimeUtil;
import com.sydac.mmrbem.dmi.fe.comms.CommsBinding;
import com.sydac.mmrbem.dmi.fe.comms.CommsBindingImpl;
...
/**
 * DMI Frontend Panel Manager that bridges the simulator session with the JavaFx
 * application's creation and presentation
 *
 * @author Nazim Oliyath
 */
public class DMIFrontendPanelManager extends AbstractFrontendPanelManager
{
  ...
  public DMIFrontendPanelManager(PeerApplication baseApplication)
  {
      ...
      int carIndex)
  {
    ...
    DMIFactory.register(carIndex, hubObjectProvider);
    DMIFactory.register(carIndex, new CommsBindingImpl(aGateway, carIndex));
  }
      ...
      BaseSessionData aSessionData, boolean aIsSimulation, IHubObjectCallback aCallback)
  {
    ...
    Map<Integer, CommsBinding> commsBindingMap = DMIFactory.getCommsBinding();
    ...
    // FrontendModule execution path
    {
      ...
      {
        int carIndex = commsBindingEntry.getKey();
        callbacks.put(carIndex, new DMICompositeHubObjectCallback(carIndex, commsBindingEntry.getValue(),
    ...
    if (aIsSimulation)
    {
      monitor = new IODataObjectCreator(aGateway, aApplicationName, logicalHubId, creatorId,
          ProjectObjectConstants.ClassId.DMI, scenarioTrainId, callbacks,
          ProjectObjectDefinition.getObjectDefinition(ProjectObjectConstants.ClassId.DMI_INPUTS),
          ProjectObjectDefinition.getObjectDefinition(ProjectObjectConstants.ClassId.DMI_OUTPUTS),
          ProjectObjectDefinition.getObjectDefinition(ProjectObjectConstants.ClassId.DMI_INPUTS_VALUES));

    }
    else
    {
      monitor = new IODataObjectMonitor(aGateway, logicalHubId, creatorId, ProjectObjectConstants.ClassId.DMI,
  ...
  public void createFxApplication(int carIndex, Properties properties)
  {
    ...
    DMIApplication.startApplication(xpos, ypos, width, height, sceneManager);
  }
  ...
  public void showFxApplication(final int carIndex, final boolean show)
  {
    if (show)
    {
      DMIApplication.showApplication(carIndex, dbScenarioTrain);
    }
    else
    {
      DMIApplication.hideApplication();
    }
  }
  ...
  {
    DMIApplication.pauseApplication(pause);
  }
   ...
   * @author Nazim Oliyath
   */
  public static final class DMICompositeHubObjectCallback implements IHubObjectCallback
  {
    ...
    public DMICompositeHubObjectCallback(int carIndex, IHubObjectCallback hubObjectCallback,
  ...
  protected static final class FxHubObjectCallback implements IHubObjectCallback
  {
    ...
    {
      hubObjectCallback.onHubInputCreation(hubInput);
      CommsListenerManager.attachToModel(carIndex, DMIFactory.getCommsBinding(carIndex));
    }
