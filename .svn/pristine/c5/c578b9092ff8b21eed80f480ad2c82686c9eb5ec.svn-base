...
/**
 * This class handles HVAC 2 Internal Temperature update from vehicle outputs.
 * 
 */
public class HVAC2InternalTemperatureVehicleOutputRequestHandler implements
    ITCMSVehicleOutputsRequestHandler<TCMSOutputsHVACSystemScreenImpl<Integer, IAppOutputsHVACSystemScreenHandler>>
{

  private List<Entry<VehicleInternalTemperatureValueDO>> internalTemperatureStatus2 = new ArrayList<>();
      ...
      TCMSOutputsHVACSystemScreenImpl<Integer, IAppOutputsHVACSystemScreenHandler> request)
  {
    try
    {
      VehicleInternalTemperatureValueDO vehicleInternalTemperatureValueDO = ITCMSVehicleOutputsRequestHandler
          .getVehicleStatusDo(internalTemperatureStatus2, request.getCarIndex(),
              VehicleInternalTemperatureValueDO.class);
      vehicleInternalTemperatureValueDO.setInternalTemperature(request.newValue);
      ITCMSVehicleOutputsRequestHandler.updateVehicleStatusDOList(internalTemperatureStatus2,
          vehicleInternalTemperatureValueDO, request.getCarIndex());
      setInternalTemperatureStatus(internalTemperatureStatus2,
          builder -> request.getHandler().setInternalTemperatureStatus2(builder.build().toByteArray()));
    }
