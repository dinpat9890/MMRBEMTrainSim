...
import com.sydac.mmrbem.tcms.fe.gui.binder.BrakeSystemTestScreenBinder;
...
public class BrakeSystemTestScreenController extends AbstractTCMSController
{

  @FXML
  private Button brakeTestUpButton;

  @FXML
  private Button brakeTestDownButtoon;

  @FXML
  private Button brakeTestStartButton;

  @FXML
  private Label brakeTestScreenLabel;

  @FXML
  private TableView<TestScreenTableDO> brakeTestScreenTable;
  ...
  public void initializeImpl()
  {
    ...
    tcmsScreenBinders.add(new BrakeSystemTestScreenBinder(testSystemScreenModel));

    currentPageListener = (observable, oldVal, newVal) -> updateBrakeSystemTestScreenTable();
    testSystemScreenModel.getCurrentPageNumber().addListener(currentPageListener);

    totalPagesListener = (observable, oldVal, newVal) -> updateBrakeSystemTestScreenTable();
    testSystemScreenModel.getTotalPageNumber().addListener(totalPagesListener);

    updateBrakeSystemTestScreenTable();
  ...
  private void setTableProperties(ListProperty<TestScreenTableDO> eventList)
  {
    ...
    brakeTestScreenTable.setFixedCellSize(CELL_HEIGHT * 1.0);

    brakeTestScreenTable.getSelectionModel().setCellSelectionEnabled(false);
    brakeTestScreenTable.getSelectionModel().clearAndSelect(0);
    brakeTestScreenTable.setPadding(new Insets(0, 0, 0, 0));

    brakeTestScreenTable.widthProperty().addListener((source, oldWidth, newWidth) -> {
      Pane header = (Pane)brakeTestScreenTable.lookup("TableHeaderRow"); //$NON-NLS-1$
      if (header.isVisible())
      {
    ...
    brakeTestScreenTable.getColumns().add(getColumn("carCode", COMPONENT_WIDTH * 1.0, "CAR")); //$NON-NLS-1$ //$NON-NLS-2$
    brakeTestScreenTable.getColumns().add(getColumn("componentName", TEST_STATUS_WIDTH * 1.0, "COMPONENT")); //$NON-NLS-1$ //$NON-NLS-2$
    brakeTestScreenTable.getColumns().add(getColumn("testStatus", LAST_UPDATE_WIDTH * 1.0, "TEST STATUS")); //$NON-NLS-1$ //$NON-NLS-2$
    brakeTestScreenTable.getColumns()
        .add(getColumn("lastUpdateDate", LAST_UPDATE_WIDTH * 1.0, "LAST UPDATE")); //$NON-NLS-1$ //$NON-NLS-2$

    brakeTestScreenTable.setColumnResizePolicy(TableView.CONSTRAINED_RESIZE_POLICY);

    brakeTestScreenTable.setItems(eventList);

    brakeTestScreenTable.getSelectionModel().selectedIndexProperty()
        .addListener(new RowChangeHandler(brakeTestScreenTable, eventList));
  ...
  private void updateBrakeSystemTestScreenTable()
  {
    brakeTestScreenLabel.setText("Showing page " + TCMSUIConstants.CURRENT_PAGE + " of " //$NON-NLS-1$ //$NON-NLS-2$
