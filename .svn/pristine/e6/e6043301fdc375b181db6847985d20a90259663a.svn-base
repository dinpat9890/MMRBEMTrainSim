...
namespace Features
{
    ...
    public:
        ...
        void setString(string property, string value)
        {
            ...
            // Process the MountType Property
            if (property == "MountType" )
            {
				if (value == "Track" || value == "TrackTunnel" || value == "GRP" || value == "GRPTunnel" )
				{	
			        Node parent_node = getParentNode();
					
					// Enable the correct surface based on the Balise position and Track type
					if (value == "Track")
					{
						ProjUtil::enableChildSurface(parent_node, "balise", 1);
						ProjUtil::enableChildSurface(parent_node, "balise_tunnel", 0);
						ProjUtil::enableChildSurface(parent_node, "grpbox", 0);
						ProjUtil::enableChildSurface(parent_node, "grpbox_tunnel", 0);
						
					}
					else if (value == "TrackTunnel")
					{
						ProjUtil::enableChildSurface(parent_node, "balise", 0);
						ProjUtil::enableChildSurface(parent_node, "balise_tunnel", 1);
						ProjUtil::enableChildSurface(parent_node, "grpbox", 0);
						ProjUtil::enableChildSurface(parent_node, "grpbox_tunnel", 0);
					}
					else if (value == "GRP")
					{
						ProjUtil::enableChildSurface(parent_node, "balise", 0);
						ProjUtil::enableChildSurface(parent_node, "balise_tunnel", 0);
						ProjUtil::enableChildSurface(parent_node, "grpbox", 1);
						ProjUtil::enableChildSurface(parent_node, "grpbox_tunnel", 0);
					}
					else if (value == "GRPTunnel")
					{
						ProjUtil::enableChildSurface(parent_node, "balise", 0);
						ProjUtil::enableChildSurface(parent_node, "balise_tunnel", 0);
						ProjUtil::enableChildSurface(parent_node, "grpbox", 0);
						ProjUtil::enableChildSurface(parent_node, "grpbox_tunnel", 1);
					}
				 	
				}
				else
				{
					log.warning("%s (line %d): Feature %s(%d) : Property '%s' has unknown value of %s. Valid Values are Track, TrackTunnel, GRP, GRPTunnel\n",__FILE__, __LINE__, getName(), getID(), property, value);	
				}
            }
            else
            {
