...
#include "DoorOpenedCmdRemovedTooQuicklyInfractionRule.h"
...
namespace
{
    constexpr auto rule_name{ "DOPRMQK" };
    //constexpr auto sec_to_millisec{ 1000 };
    namespace Parameters
    {
        constexpr auto penalty{ "DOPRMQK_P" };
        constexpr auto min_press_time{ "MIN_PRS_TM" };
    }
}

DoorOpenedCmdRemovedTooQuicklyInfractionRule::DoorOpenedCmdRemovedTooQuicklyInfractionRule(
    InfractionUnit&                    unit,
  ...
  : InfractionRule(unit, rule_name, scoring_parameters),
    ...
    m_min_press_time(Scoring::GetNonNegativeParameter(scoring_parameters, Parameters::min_press_time)),
...
void DoorOpenedCmdRemovedTooQuicklyInfractionRule::CheckInfraction()
{
    ...
    if (is_door_control_mode_manual)
    {
        ...
        {
            //auto left_button_press_time = (m_left_button_release_time_in_seconds.time * sec_to_millisec - m_left_button_press_time_in_seconds.time * sec_to_millisec);
            ...
            if (left_button_press_time < m_min_press_time)
            {
                Trace::GetLogger().STAMP(Trace::VERBOSE) << "Left door open command removed too quickly.. "  << Trace::EndLog();
        ...
        {
            //auto right_button_press_time = (m_right_button_release_time_in_seconds.time * sec_to_millisec - m_right_button_press_time_in_seconds.time * sec_to_millisec);
            ...
            if (right_button_press_time < m_min_press_time)
            {
                Trace::GetLogger().STAMP(Trace::VERBOSE) << "Right door open command removed too quickly. " << Trace::EndLog();
                RecordInfraction(m_penalty);
            }
        }
    }
