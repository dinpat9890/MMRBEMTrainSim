...
public class BrakeLoopInterlockScreenController extends AbstractTCMSController
{
  ...
  private static final String EIGHT_CAR_FXML_NAME = "BrakeLoopInterlockScreenEightCar.fxml"; //$NON-NLS-1$
  ...
  private void updateCars(int noOfCars)
  {
    ...
    if (noOfCars != TCMSUIConstants.NUMBER_OF_CARS_ZERO)
    {
      ...
      if (noOfCars == TCMSUIConstants.NUMBER_OF_CARS_EIGHT)
      {

        BrakeLoopInterlockEightCarScreenController systemCommandSixCar = new BrakeLoopInterlockEightCarScreenController();
        outerGrid.getChildren().add(systemCommandSixCar.createCar(EIGHT_CAR_FXML_NAME));
  ...
  {
    if (ScreenId.BRAKE_LOOP_INTERLOCKS_SCREEN.getId() == screenId)
    {
      updateStatus();
    }
  }
