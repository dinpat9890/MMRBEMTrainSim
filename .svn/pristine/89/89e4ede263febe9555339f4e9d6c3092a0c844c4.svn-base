#include "TrainStoppedAfterStoppingPointInfractionRule.h"
...
namespace
{
	constexpr auto rule_name{ "STPOSAFT" };

	namespace Parameters
	{
		constexpr auto penalty{ "STPOSAFT_P" };
		constexpr auto max_distance{ "MAX_DIST2" };
		constexpr auto min_distance{ "MIN_DIST2" };
	}
	const double MIN_MOVING_SPEED = 0.1;
	const double DISTANCE_BETWEEN_6C_AND_8C_STOPPING_POINT = 22.92;
}

TrainStoppedAfterStoppingPointInfractionRule::TrainStoppedAfterStoppingPointInfractionRule(
	InfractionUnit&                    unit,
...
void TrainStoppedAfterStoppingPointInfractionRule::SpeedChanged(Sim::Number number)
{
	...
	if (change_in_train_stop_state && train_has_stopped )
	{
		...
		{
			//If stopping point is between m_min_distance to m_max_distance before lead car
			if (IsTrainNearToFeature(-m_max_distance, 0.0 ) && !IsTrainNearToFeature( -m_min_distance, 0.0))
			{
				...
				//There may be possibility that 6C car can stop after 8C stopping point 
				//or 8C car can stop after 6C stopping point & before 8C stopping point.
				...
				//If 6C train is stopped && there is 1 stopping points back
				if (is_6C_train && 1 == vect_comms_obj_stoppingpoint.size())
				{
					Trace::GetLogger().STAMP(Trace::INFO) << "Train stopped after stopping point infraction. " << Trace::EndLog();
					RecordInfraction(m_penalty);
				}
				//If 8C train is stopped && there are 2 stopping points back
				else if (is_8C_train && 2 == vect_comms_obj_stoppingpoint.size())
				{
					Trace::GetLogger().STAMP(Trace::INFO) << "Train stopped after stopping point infraction. " << Trace::EndLog();
					RecordInfraction(m_penalty);
				}
                //If train is other than 6C or 8C && is stopped && there is 1 stopping points behind ==> Expected to stop at 6C stopping point
                else if ((!is_6C_train && !is_8C_train) && 1 == vect_comms_obj_stoppingpoint.size())
                {
