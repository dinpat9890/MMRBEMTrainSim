...
/**
 * This class handles HVAC 2 Operating Mode Status update from vehicle outputs
 * 
 */
public class HVACOpModeStatus2VehicleOutputsRequestHandler implements
    ITCMSVehicleOutputsRequestHandler<TCMSOutputsHVACSystemScreenImpl<Integer, IAppOutputsHVACSystemScreenHandler>>
{

  protected final List<Entry<VehicleHVACOpModeStatusDO>> hvacOpModeStatus2 = new ArrayList<>();
  protected final List<Entry<VehicleHVACOpModeStatusDO>> hvacControlStatus2 = new ArrayList<>();
      ...
      TCMSOutputsHVACSystemScreenImpl<Integer, IAppOutputsHVACSystemScreenHandler> request)
  {
    try
    {
      VehicleHVACOpModeStatusDO vehicleHVACOpModeStatusDO = ITCMSVehicleOutputsRequestHandler
          .getVehicleStatusDo(hvacOpModeStatus2, request.getCarIndex(), VehicleHVACOpModeStatusDO.class);
      setStatus(request.getPropertyKey(), request.getNewValue(), vehicleHVACOpModeStatusDO);
      ITCMSVehicleOutputsRequestHandler.updateVehicleStatusDOList(hvacOpModeStatus2,
          vehicleHVACOpModeStatusDO, request.getCarIndex());
      
      ITCMSVehicleOutputsRequestHandler.updateVehicleStatusDOList(hvacControlStatus2,
          vehicleHVACOpModeStatusDO, request.getCarIndex());
      setHVACOpModeStatus(hvacOpModeStatus2, request.getVehicleDetailModelList(),
          builder -> request.getHandler().setHvacOperatingModeStatus2(builder.build().toByteArray()));
  
      setHvacControlModeStatus(hvacControlStatus2, request.getVehicleDetailModelList(),
          builder -> request.getHandler().setHvacControlModeStatus2(builder.build().toByteArray()));
    }
  ...
  {
    if (propertyKey.equals(ITCMSVehicleOutputsHVACSystemScreenProperties.HVAC_2_FAULT))
    {
      vehicleHVACOpModeStatusDO.setHvacFault(newvalue);
    }
    else if (propertyKey.equals(ITCMSVehicleOutputsHVACSystemScreenProperties.HVAC_2_ISOLATED))
    {
      vehicleHVACOpModeStatusDO.setHvacIsolated(newvalue);
    }
    else if (propertyKey.equals(ITCMSVehicleOutputsHVACSystemScreenProperties.HVAC_2_STATUS))
    {
      vehicleHVACOpModeStatusDO.setHvacStatus(newvalue);
    }else if (propertyKey.equals(ITCMSVehicleOutputsHVACSystemScreenProperties.HVAC_2_REDUCED_MODE))
    {
