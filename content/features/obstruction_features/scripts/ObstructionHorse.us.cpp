...
#ifndef OBSTRUCTION_HORSE_US_CPP
#define OBSTRUCTION_HORSE_US_CPP

#include <AnimatedFeature.us.cpp>

namespace Features
{
    /// Horse obstruction feature
    class ObstructionHorse : AnimatedFeature
    {
    private:
        int m_valid;
        enum
    ...
    public:
        ...
        /// Constructs a new instance of a ObstructionHorse feature. The constructor
        ...
        ObstructionHorse(FeatureInfoExtern info, Properties properties, Node parent)
        {
            __AnimatedFeature__(info, properties, parent);

            // Attempt to load the horse node. If we are successful loading it, register
            // with the AnimatedFeature class and then associate each state with its set of actions.
            Node horse                      = engine.world.loadNode("horse.node");
            ObjectMeshSkinned skinned_horse = (horse == 0) ? 0 : node_cast(horse);
            if(skinned_horse != 0)
            {
                registerNode(skinned_horse);

                int horse_idle_animation_id     = -1; // Horse Idle
                int horse_walk_animation_id     = -1;   // Horse Walk

                horse_idle_animation_id = skinned_horse.addAnimation("horse_idle.anim");
                horse_walk_animation_id = skinned_horse.addAnimation("horse_walk.anim");

                // Now define the state transitions 1 - Not Visible 2 - Off Track 3 - On Track.
                setAction(NOT_VISIBLE, new NodeEnable(skinned_horse, 0));
                setAction(OFF_TRACK, new NodeActionSequence((new ObstructionMoveAndAnimate(skinned_horse, horse_walk_animation_id, 45.0, 2.0, 180.0, translate(dvec3(-5.0, 0.0, 0.0)) * rotateZ(180.0)), new NodeActionLoop(new ObstructionNodeAnimate(skinned_horse, horse_idle_animation_id, 15.0)))));
                setAction(ON_TRACK, new NodeActionSequence((new ObstructionMoveAndAnimate(skinned_horse, horse_walk_animation_id, 45.0, 2.0, 180.0, translate(dvec3(0.0, 0.0, 0.0)) * rotateZ(180.0)), new NodeActionLoop(new ObstructionNodeAnimate(skinned_horse, horse_idle_animation_id, 15.0)))));
                m_valid = 1;
            }
            else
            {
                log.error("Unable to load horse obstruction - invalid node file for feature with id %d\n", info.getID());
                m_valid = 0;
            }
        }
        ...
        /// This class allows us to move and animate obstruction (in this case it is horse). This a specific case of
        ...
        /// This class represents the action to animate the given obstruction node horse (which is
    ...
    /// feature type (ObstructionHorse).
    class ObstructionHorseType : FeatureType
    {
    public:
        ...
        string getName()
        {
            return "ObstructionHorse";
        }

        /// Factory function - creates a ObstructionHorse using FeatureInfo and Properties.
        ///
        ...
        {
            return new ObstructionHorse(info, properties, parent);
        }
    };
}
