...
#ifndef OBSTRUCTION_DOG_US_CPP
#define OBSTRUCTION_DOG_US_CPP

#include <AnimatedFeature.us.cpp>

namespace Features
{
    /// Dog obstruction feature 
    class ObstructionDog : AnimatedFeature
    {
    private:
        int m_valid;
        enum
    ...
    public:
        /// Constructs a new instance of a ObstructionDog feature. The constructor
        ...
        ObstructionDog(FeatureInfoExtern info, Properties properties, Node parent)
        {
            __AnimatedFeature__(info, properties, parent);

            // Attempt to load the dog node. If we are successful loading it, register
            // with the AnimatedFeature class and then associate each state with its set of actions.
            Node dog                      = engine.world.loadNode("dog.node");
            ObjectMeshSkinned skinned_dog = (dog == 0) ? 0 : node_cast(dog);
            if(skinned_dog != 0)
            {
                registerNode(skinned_dog);

                int dog_idle_animation_id       = -1;   // Dog Idle
                int dog_walk_animation_id       = -1;   // Dog Walk

                dog_idle_animation_id = skinned_dog.addAnimation("dog_idle.anim");
                dog_walk_animation_id = skinned_dog.addAnimation("dog_walk.anim");

                // Now define the state transitions 1 - Not Visible 2 - Off Track 3 - On Track.
                setAction(NOT_VISIBLE, new NodeEnable(skinned_dog, 0));
                setAction(OFF_TRACK, new NodeActionSequence((new ObstructionMoveAndAnimate(skinned_dog, dog_walk_animation_id, 45.0, 2.0, 180.0, translate(dvec3(-5.0, 0.0, 0.0)) * rotateZ(180.0)), new NodeActionLoop(new ObstructionNodeAnimate(skinned_dog, dog_idle_animation_id, 15.0)))));
                setAction(ON_TRACK, new NodeActionSequence((new ObstructionMoveAndAnimate(skinned_dog, dog_walk_animation_id, 45.0, 2.0, 180.0, translate(dvec3(0.0, 0.0, 0.0)) * rotateZ(180.0)), new NodeActionLoop(new ObstructionNodeAnimate(skinned_dog, dog_idle_animation_id, 15.0)))));
                m_valid = 1;
            }
            else
            {
                log.error("Unable to load Dog obstruction - invalid node file for feature with id %d\n", info.getID());
                m_valid = 0;
            }
        }
        ...
        /// This class allows us to move and animate obstruction (in this case it is Dog). This a specific case of
        ...
        /// This class represents the action to animate the given obstruction node Dog (which is
    ...
    /// feature type (ObstructionDog).
    class ObstructionDogType : FeatureType
    {
    public:
        ...
        string getName()
        {
            return "ObstructionDog";
        }

        /// Factory function - creates a ObstructionDog using FeatureInfo and Properties.
        ///
        ...
        {
            return new ObstructionDog(info, properties, parent);
        }
    };
}
