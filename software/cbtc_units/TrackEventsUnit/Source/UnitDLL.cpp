...
///
/// @copyright  (C) Copyright:  Common Pty Ltd, 2015
///
    ...
    if(!gateway.IsReplay() && !session_data.preview)
    {
        ...
        for(const auto scenario_train_id : train_ids)
        {
            ...
            if (scenario_train.is_simulated)
            {
                ...
                assert(resources.Exists<WorldAPI::ScenarioTrainPathWatcher>(scenario_train_id));

                ret.push_back([&, scenario_train_id]() 
                        { 
                            return std::make_shared<TrackEventsUnit>(gateway,
                                                                ...
                                                                TrackEventProperties(*g_properties).GetMaxLookAhead(),
                                                                TrackEventProperties(*g_properties).GetTransitionZoneLength(),
                                                                scenario_train_id); 
                        });           
            }
        }
