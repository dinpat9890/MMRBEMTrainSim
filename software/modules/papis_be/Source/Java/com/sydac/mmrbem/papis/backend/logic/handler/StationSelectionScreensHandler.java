...
public class StationSelectionScreensHandler extends AbstractFunctionalLogic implements IScreenChangeListener
{
  ...
  protected StationSelectionScreensHandler(CommsBinding commsBinding, SharedDataModel sharedDataModel)
  {
  ...
  public <T> void appInputsPropertyUpdated(PropertyKey<T> key, T oldValue, T newValue, int vehicleIndex)
  {
    ...
    if (sharedDataModel.getScreenId() == ScreenId.START_STATION_SELECTION_SCREEN.getId())
    {
      ...
      else if (IAppInputsStationSettingScreensProperties.SELECTED_START_ST_ID_REQUEST.equals(key)
          && (Integer)newValue > 0)
      {
        setInputData(propertySetter, IPISBEInputsStationSettingScreensProperties.START_STATION_ID,
            (Integer)newValue);
      }
      ...
      else if (IAppInputsStationSettingScreensProperties.START_STATION_SELECTED.equals(key)
          && (Integer)newValue > 0)
      {
        startStationIndex = (Integer)newValue;
      }
    }
  }
  ...
  private void decrementCounterMethod()
  {
    ...
    {
      startStationIndex = listSizePerPage - 1;
      prevPage();

    }

    setApplicationOutputsProperty(IAppOutputsCommonProperties.START_STATION_SELECTED_BUTTON,
        startStationIndex);
  }

  private void incrementCounterMethod()
  {
    if (startStationIndex == listSizePerPage - 1)
    {
      startStationIndex = ApplicationConstants.RESET_SELECTED_INDEX;
      nextPage();
    }
    else if (startStationIndex < stationList.size() - 1)
    {
      startStationIndex++;
    }
    setApplicationOutputsProperty(IAppOutputsCommonProperties.START_STATION_SELECTED_BUTTON,
  ...
  private void allStationList()
  {
    ...
    setApplicationOutputsProperty(
        IAppOutputsManualModeSettingScreenProperties.MANUAL_MODE_SCREEN_ALL_STATION_LIST,
