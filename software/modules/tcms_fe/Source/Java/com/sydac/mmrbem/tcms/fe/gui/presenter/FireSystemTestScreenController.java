...
import com.sydac.mmrbem.tcms.fe.gui.binder.FireSystemTestScreenBinder;
...
public class FireSystemTestScreenController extends AbstractTCMSController
{

  @FXML
  private Button fireTestUpButton;

  @FXML
  private Button fireTestDownButton;

  @FXML
  private Button fireTestStartButton;

  @FXML
  private Label fireTestScreenLabel;

  @FXML
  private TableView<TestScreenTableDO> fireTestScreenTable;
  ...
  public void initializeImpl()
  {
    ...
    tcmsScreenBinders.add(new FireSystemTestScreenBinder(testSystemScreenModel));

    currentPageListener = (observable, oldVal, newVal) -> updateFireSystemTestScreenTable();
    testSystemScreenModel.getCurrentPageNumber().addListener(currentPageListener);

    totalPagesListener = (observable, oldVal, newVal) -> updateFireSystemTestScreenTable();
    testSystemScreenModel.getTotalPageNumber().addListener(totalPagesListener);

    updateFireSystemTestScreenTable();
  ...
  private void setTableProperties(ListProperty<TestScreenTableDO> eventList)
  {
    ...
    fireTestScreenTable.setFixedCellSize(CELL_HEIGHT * 1.0);

    fireTestScreenTable.getSelectionModel().setCellSelectionEnabled(false);
    fireTestScreenTable.getSelectionModel().clearAndSelect(0);
    fireTestScreenTable.setPadding(new Insets(0, 0, 0, 0));

    fireTestScreenTable.widthProperty().addListener((source, oldWidth, newWidth) -> {
      Pane header = (Pane)fireTestScreenTable.lookup("TableHeaderRow"); //$NON-NLS-1$
      if (header.isVisible())
      {
    ...
    fireTestScreenTable.getColumns().add(getColumn("carCode", COMPONENT_WIDTH * 1.0, "CAR")); //$NON-NLS-1$ //$NON-NLS-2$
    fireTestScreenTable.getColumns().add(getColumn("componentName", TEST_STATUS_WIDTH * 1.0, "COMPONENT")); //$NON-NLS-1$ //$NON-NLS-2$
    fireTestScreenTable.getColumns().add(getColumn("testStatus", LAST_UPDATE_WIDTH * 1.0, "TEST STATUS")); //$NON-NLS-1$ //$NON-NLS-2$
    fireTestScreenTable.getColumns().add(getColumn("lastUpdateDate", LAST_UPDATE_WIDTH * 1.0, "LAST UPDATE")); //$NON-NLS-1$ //$NON-NLS-2$

    fireTestScreenTable.setColumnResizePolicy(TableView.CONSTRAINED_RESIZE_POLICY);

    fireTestScreenTable.setItems(eventList);

    fireTestScreenTable.getSelectionModel().selectedIndexProperty()
        .addListener(new RowChangeHandler(fireTestScreenTable, eventList));
  ...
  private void updateFireSystemTestScreenTable()
  {
    fireTestScreenLabel.setText("Showing page " + TCMSUIConstants.CURRENT_PAGE + " of " //$NON-NLS-1$ //$NON-NLS-2$
