...
import com.sydac.mmrbem.tcms.fe.gui.binder.HVACSystemTestScreenBinder;
...
public class HVACSystemTestScreenController extends AbstractTCMSController
{

  @FXML
  private Button HVACTestUpButton;

  @FXML
  private Button HVACTestDownButtoon;

  @FXML
  private Button HVACTestStartButton;

  @FXML
  private Label HVACTestScreenLabel;

  @FXML
  private TableView<TestScreenTableDO> HVACTestScreenTable;
  ...
  public void initializeImpl()
  {
    ...
    tcmsScreenBinders.add(new HVACSystemTestScreenBinder(testSystemScreenModel));

    currentPageListener = (observable, oldVal, newVal) -> updateHVACSystemTestTable();
    testSystemScreenModel.getCurrentPageNumber().addListener(currentPageListener);

    totalPagesListener = (observable, oldVal, newVal) -> updateHVACSystemTestTable();
    testSystemScreenModel.getTotalPageNumber().addListener(totalPagesListener);

    updateHVACSystemTestTable();
  ...
  private void setTableProperties(ListProperty<TestScreenTableDO> eventList)
  {
    ...
    HVACTestScreenTable.setFixedCellSize(CELL_HEIGHT * 1.0);

    HVACTestScreenTable.getSelectionModel().setCellSelectionEnabled(false);
    HVACTestScreenTable.getSelectionModel().clearAndSelect(0);
    HVACTestScreenTable.setPadding(new Insets(0, 0, 0, 0));

    HVACTestScreenTable.widthProperty().addListener((source, oldWidth, newWidth) -> {
      Pane header = (Pane)HVACTestScreenTable.lookup("TableHeaderRow"); //$NON-NLS-1$
      if (header.isVisible())
      {
    ...
    HVACTestScreenTable.getColumns().add(getColumn("carCode", COMPONENT_WIDTH * 1.0, "CAR")); //$NON-NLS-1$ //$NON-NLS-2$
    HVACTestScreenTable.getColumns().add(getColumn("componentName", TEST_STATUS_WIDTH * 1.0, "COMPONENT")); //$NON-NLS-1$ //$NON-NLS-2$
    HVACTestScreenTable.getColumns().add(getColumn("testStatus", LAST_UPDATE_WIDTH * 1.0, "TEST STATUS")); //$NON-NLS-1$ //$NON-NLS-2$
    HVACTestScreenTable.getColumns().add(getColumn("lastUpdateDate", LAST_UPDATE_WIDTH * 1.0, "LAST UPDATE")); //$NON-NLS-1$ //$NON-NLS-2$

    HVACTestScreenTable.setColumnResizePolicy(TableView.CONSTRAINED_RESIZE_POLICY);

    HVACTestScreenTable.setItems(eventList);

    HVACTestScreenTable.getSelectionModel().selectedIndexProperty()
        .addListener(new RowChangeHandler(HVACTestScreenTable, eventList));
  ...
  private void updateHVACSystemTestTable()
  {
    HVACTestScreenLabel .setText("Showing page " + TCMSUIConstants.CURRENT_PAGE + " of " //$NON-NLS-1$ //$NON-NLS-2$
