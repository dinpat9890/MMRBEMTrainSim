...
public class FrontendPanelManager extends AbstractFrontendPanelManager
{
  ...
  public FrontendPanelManager(PeerApplication baseApplication, Frontend frontend)
  {
      ...
      int carIndex)
  {
    ...
    TCMSFactory.register(carIndex, hubObjectProvider);
    TCMSFactory.register(carIndex, new CommsBindingImpl(aGateway, carIndex));
  }
      ...
      BaseSessionData aSessionData, boolean aIsSimulation, IHubObjectCallback aCallback)
  {
    ...
    Map<Integer, CommsBinding> commsBindingMap = TCMSFactory.getCommsBinding();
    ...
    if (aIsSimulation)
    {
      monitor = new IODataObjectCreator(aGateway, aApplicationName, logicalHubId, creatorId,
          ProjectObjectConstants.ClassId.VDU, scenarioTrainId, callbacks,
          ProjectObjectDefinition.getObjectDefinition(ProjectObjectConstants.ClassId.VDU_INPUTS),
          ProjectObjectDefinition.getObjectDefinition(ProjectObjectConstants.ClassId.VDU_OUTPUTS),
          ProjectObjectDefinition.getObjectDefinition(ProjectObjectConstants.ClassId.VDU_INPUTS_VALUES));

    }
    else
    {
      monitor = new IODataObjectMonitor(aGateway, logicalHubId, creatorId, ProjectObjectConstants.ClassId.VDU,
  ...
  public void createFxApplication(int carIndex, Properties properties)
  {
    ...
    TCMSApplication.startApplication(xpos, ypos, width, height, sceneManager);
  }
  ...
  public void showFxApplication(final int carIndex, final boolean show)
  {
    if (show)
    {
      TCMSApplication.showApplication(carIndex, frontend);
    }
    else
    {
      TCMSApplication.hideApplication();
    }
  }
  ...
  {
    TCMSApplication.pauseApplication(pause);
  }
  ...
  class ScreenChangeListener extends AbstractCommsListener
  {
    ...
    {
      if (frontend == Frontend.TCMS && key == IAppOutputsScreenChangeProperties.TCMS_CURRENT_SCREEN_ID)
      {
        updateScene((Integer)newValue);
      }
      else if (frontend == Frontend.CCTV && key == IAppOutputsScreenChangeProperties.CCTV_CURRENT_SCREEN_ID)
      {
        updateScene((Integer)newValue);
      }
    }

    private void updateScene(Integer newValue)
    {
      // Need to change getFiveCarScreenName to correct screen mapping
      // according to number of cars
      String screenName;
      screenName = getScreenName(newValue);
      if (fxPanels.containsKey(scenarioCarIndex))
      {
        Scene scene = fxPanels.get(scenarioCarIndex).changeScene(screenName, scenarioCarIndex, sceneManager);
        if (hideMouse)
        {
  ...
  protected static final class FxHubObjectCallback implements IHubObjectCallback
  {
    ...
    {
      hubObjectCallback.onHubInputCreation(hubInput);
      CommsListenerManager.attachToModel(carIndex, TCMSFactory.getCommsBinding(carIndex));
    }
