...
public class HVCommandTMiddleCarController extends AbstractTCMSController
{
  ...
  @FXML
  private Button pantoSelectMidBtn;
  ...
  private ChangeListener<Number> midPAntoListener;

  private ChangeListener<Number> midPantoLowListener;
  ...
  public HVCommandTMiddleCarController(CarConfigurationModel carConfig,
      HighVoltageScreenModel highVoltageScreenModel,
      HighVoltageMiddleTCarScreenModel highVoltageMiddleTCarScreenModel)
  {
    this.highVoltageScreenModel = highVoltageScreenModel;
    this.highVoltageMiddleTCarScreenModel = highVoltageMiddleTCarScreenModel;
  ...
  public void initializeImpl()
  {
    ...
    pantoSelectButtonProperty = new NodePseudoClassProperty(pantoSelectMidBtn);
  ...
  //  highVoltageScreenModel.getReqCtrlVcbNsStatusProperty().addListener(reqCtrlVcbNsListener);
    ...
    // highVoltageScreenModel.getHvSelectedCarIndexList().addListener(carConfigDataListener);

    midPAntoListener = (observable, oldValue, newValue) -> checkPantoSelected();
    highVoltageMiddleTCarScreenModel.getMiddlePantoState().addListener(midPAntoListener);

    midPantoLowListener = (observable, oldValue, newValue) -> pantoLowerStateUpdate();
    highVoltageMiddleTCarScreenModel.getMiddlePantoLowState().addListener(midPantoLowListener);

    resetPantoListener = (observable, oldValue, newValue) -> pantoResetUpdate(newValue.intValue());
    highVoltageMiddleTCarScreenModel.getMidResetPantoState().addListener(resetPantoListener);

    resetVcbListener = (observable, oldValue, newValue) -> vcbResetUpdate(newValue.intValue());
    highVoltageMiddleTCarScreenModel.getMidVcbResetState().addListener(resetVcbListener);

    isolateVcbListener = (observable, oldValue, newValue) -> vcbIsolateUpdate(newValue.intValue());
    highVoltageMiddleTCarScreenModel.getMidVcbIsolateState().addListener(isolateVcbListener);

    aps1Listener = (observable, oldValue, newValue) -> aps1Update(newValue.intValue());
    highVoltageMiddleTCarScreenModel.getAps3IsolateState().addListener(aps1Listener);

    aps2Listener = (observable, oldValue, newValue) -> aps2Update(newValue.intValue());
    highVoltageMiddleTCarScreenModel.getAps4IsolateState().addListener(aps2Listener);
  ...
  private void pantoSelectStateUpdate()
  {
    ...
    if (Objects.equals(pantoState.get().getComponentState(), Pantograph.RAISED.toString().toLowerCase()))
    {
      ...
      pantoSelectMidBtn.setText(TCMSUIConstants.SELECT_TXT);
    }
//    else
//    {
//      checkPantoSelected();
//    }
  ...
  private void checkPantoSelected()
  {
    ...
    if (highVoltageMiddleTCarScreenModel.getMiddlePantoState().get() == 1)
    {
      pantoSelectButtonProperty.set(TCMSUIConstants.COMMAND_SELECTED_CSS);
      pantoSelectMidBtn.setText(TCMSUIConstants.DESELECT_TXT);
    }
    else
    {
  ...
  private void pantoLowerStateUpdate()
  {
    ...
    {
      if (highVoltageMiddleTCarScreenModel.getMiddlePantoLowState().get() == 1)
      {
  ...
  private void pantoSelectButtonAction(ActionEvent event)
  {
    if (!Objects.equals(pantoState.get().getComponentState(), Pantograph.RAISED.toString().toLowerCase()))
    {
      ...
      if (highVoltageMiddleTCarScreenModel.getMiddlePantoState().get() == 1)
      {
        TCMSFactory.getServiceRequestHandler(scenarioCarIndex).sendRequest(VDUInputs.PANTO_BUTTON_MIDDLE_REQ,
            0);
      }
      else
      {
  ...
  private void pantoLowerButtonAction(ActionEvent event)
  {
        ...
        && Objects.equals(vcbState.get().getComponentState(), VCBState.OPEN.toString().toLowerCase()))
    {
      ...
      TCMSFactory.getServiceRequestHandler(scenarioCarIndex).sendHVCommandReq(VDUInputs.PANTO_LOWER_REQ_CAR,
          carIndex);
      TCMSFactory.getServiceRequestHandler(scenarioCarIndex)
          .sendButtonClickRequest(VDUInputs.PANTO_LOW_BUTTON_MIDDLE_REQ);
  ...
  private void pantoResetButtonAction(ActionEvent event)
  {
    TCMSFactory.getServiceRequestHandler(scenarioCarIndex).sendHVCommandReq(VDUInputs.PANTO_RESET_REQ_CAR,
        carIndex);
    TCMSFactory.getServiceRequestHandler(scenarioCarIndex)
        .sendButtonClickRequest(VDUInputs.PANTO_RESET_BUTTON_MIDDLE_REQ);
  }
  ...
  private void vcbResetButtonAction(ActionEvent event)
  {
    TCMSFactory.getServiceRequestHandler(scenarioCarIndex).sendHVCommandReq(VDUInputs.VCB_RESET_REQ_CAR,
        carIndex);
    TCMSFactory.getServiceRequestHandler(scenarioCarIndex)
        .sendButtonClickRequest(VDUInputs.VCB_RESET_BUTTON_MIDDLE_REQ);
  }
  ...
  private void vcbIsolateButtonAction(ActionEvent event)
  {
    ...
    TCMSFactory.getServiceRequestHandler(scenarioCarIndex)
        .sendButtonClickRequest(VDUInputs.ISOLATE_MIDDLE_VCB_REQ);
  }
  ...
  private void aps1IsolateButtonAction(ActionEvent event)
  {
    ...
    TCMSFactory.getServiceRequestHandler(scenarioCarIndex)
        .sendButtonClickRequest(VDUInputs.ISOLATE_APS3_REQUEST);
  }
  ...
  private void aps2IsolateButtonAction(ActionEvent event)
  {
    ...
    TCMSFactory.getServiceRequestHandler(scenarioCarIndex)
        .sendButtonClickRequest(VDUInputs.ISOLATE_APS4_REQUEST);
  }
