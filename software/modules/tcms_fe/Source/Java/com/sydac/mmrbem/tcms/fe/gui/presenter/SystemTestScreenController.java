...
import com.sydac.mmrbem.tcms.fe.gui.binder.SystemTestScreenBinder;
...
public class SystemTestScreenController extends AbstractTCMSController
{

  @FXML
  private Button systemTestUpButton;

  @FXML
  private Button systemTestDownButtoon;

  @FXML
  private Label systemTestScreenLabel;

  @FXML
  private TableView<TestScreenTableDO> systemTestScreenTable;
  ...
  private static final double LAST_UPDATE_WIDTH = 684;

  @Override
  public void initializeImpl()
  {
    testSystemScreenModel = new TestSystemScreenModel(scenarioCarIndex);
    tcmsScreenBinders.add(new SystemTestScreenBinder(testSystemScreenModel));

    currentPageListener = (observable, oldVal, newVal) -> updateSystemTestTable();
    testSystemScreenModel.getCurrentPageNumber().addListener(currentPageListener);

    totalPagesListener = (observable, oldVal, newVal) -> updateSystemTestTable();
    testSystemScreenModel.getTotalPageNumber().addListener(totalPagesListener);

    updateSystemTestTable();
  ...
  private void setTableProperties(ListProperty<TestScreenTableDO> eventList)
  {
    ...
    systemTestScreenTable.setFixedCellSize(CELL_HEIGHT * 1.0);

    systemTestScreenTable.getSelectionModel().setCellSelectionEnabled(false);
    systemTestScreenTable.getSelectionModel().clearAndSelect(0);
    systemTestScreenTable.setPadding(new Insets(0, 0, 0, 0));

    systemTestScreenTable.widthProperty().addListener((source, oldWidth, newWidth) -> {
      Pane header = (Pane)systemTestScreenTable.lookup("TableHeaderRow"); //$NON-NLS-1$
      if (header.isVisible())
      {
    ...
    systemTestScreenTable.getColumns().add(getColumn("componentName", TEST_STATUS_WIDTH * 1.0, "SYSTEM")); //$NON-NLS-1$ //$NON-NLS-2$
    systemTestScreenTable.getColumns().add(getColumn("testStatus", LAST_UPDATE_WIDTH * 1.0, "TEST STATUS")); //$NON-NLS-1$ //$NON-NLS-2$
    systemTestScreenTable.getColumns()
        .add(getColumn("lastUpdateDate", LAST_UPDATE_WIDTH * 1.0, "UPDATE TIME")); //$NON-NLS-1$ //$NON-NLS-2$

    systemTestScreenTable.setColumnResizePolicy(TableView.CONSTRAINED_RESIZE_POLICY);

    systemTestScreenTable.setItems(eventList);

    systemTestScreenTable.getSelectionModel().selectedIndexProperty()
        .addListener(new RowChangeHandler(systemTestScreenTable, eventList));
  ...
  private void updateSystemTestTable()
  {
    systemTestScreenLabel
