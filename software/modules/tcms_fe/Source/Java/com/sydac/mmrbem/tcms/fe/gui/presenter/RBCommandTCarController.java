...
public class RBCommandTCarController extends AbstractTCMSController
{
  ...
  @FXML
  private Button openBtnAPSFront;

  @FXML
  private Button closeBtnAPSFront;
  ...
  private NodePseudoClassProperty openBtnApsFrontProperty;

  private NodePseudoClassProperty closeBtnApsFrontProperty;
  ...
  ObjectPropertyBase<CarComponentStatesDO> apsicbState;

  public RBCommandTCarController(CarConfigurationModel carConfig, RbCommandScreenModel rbCommandScreenModel)
  {
  ...
  public void initializeImpl()
  {
    ...
    openBtnApsFrontProperty = new NodePseudoClassProperty(openBtnAPSFront);
    closeBtnApsFrontProperty = new NodePseudoClassProperty(closeBtnAPSFront);
    ...
    apsicbState = rbCommandScreenModel.apsicStatusProperty(carIndex);
    apsicbState.addListener(observable -> apsStatusUpdated());
  ...
  @FXML
  private void openBtnApsFrontAction(ActionEvent event)
  {

    if (Objects.equals(apsicbState.get().getComponentState(), APSCIBState.OPEN.toString().toLowerCase()))
    {
      TCMSFactory.getServiceRequestHandler(scenarioCarIndex).sendRequest(VDUInputs.RB_APSIC_OPEN_REQ, 1);
    }
    else
    {

      TCMSFactory.getServiceRequestHandler(scenarioCarIndex).sendRequest(VDUInputs.RB_APSIC_OPEN_REQ, 0);
    }
    TCMSFactory.getServiceRequestHandler(scenarioCarIndex).sendHVCommandReq(VDUInputs.RB_APSIC_OPEN_REQ_CAR,
        carIndex);
  }

  @FXML
  private void closeBtnApsFrontAction(ActionEvent event)
  {
    if (Objects.equals(apsicbState.get().getComponentState(), APSCIBState.CLOSED.toString().toLowerCase()))
    {
      TCMSFactory.getServiceRequestHandler(scenarioCarIndex).sendRequest(VDUInputs.RB_APSIC_COSE_REQ, 1);
    }
    else
    {

      TCMSFactory.getServiceRequestHandler(scenarioCarIndex).sendRequest(VDUInputs.RB_APSIC_COSE_REQ, 0);
    }
    TCMSFactory.getServiceRequestHandler(scenarioCarIndex).sendHVCommandReq(VDUInputs.RB_APSIC_CLOSE_REQ_CAR,
        carIndex);
  }
  ...
  private void apsStatusUpdated()
  {

    if (Objects.equals(rbCommandScreenModel.apsicStatusProperty(carIndex).get().getComponentState(),
        APSCIBState.OPEN.toString().toLowerCase()))
    {
      openBtnApsFrontProperty.set(TCMSUIConstants.COMMAND_AVAILABLE_CSS);
      closeBtnApsFrontProperty.set(TCMSUIConstants.COMMAND_NOT_AVAILABLE_CSS);
    }
    else
    {
      openBtnApsFrontProperty.set(TCMSUIConstants.COMMAND_NOT_AVAILABLE_CSS);
      closeBtnApsFrontProperty.set(TCMSUIConstants.COMMAND_AVAILABLE_CSS);
    }
