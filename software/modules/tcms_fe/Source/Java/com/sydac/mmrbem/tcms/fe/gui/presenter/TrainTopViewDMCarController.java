...
public class TrainTopViewDMCarController extends AbstractTrainTopViewPanelController
{
  @FXML
  private Label propulsionIcon;

  @FXML
  private Label doorDetrainmentIcon;

  private String carOrientation;

  public TrainTopViewDMCarController(CarConfigurationModel carConfig,
      TrainTopViewComponentStatusModel carComponentStatusModel, FooterPanelModel footerPanelModel)
  {
    ...
    this.carOrientation = carConfig.getCarOrientation();
  }
  ...
  public void initializeImpl()
  {
    ...
    NodePseudoClassProperty propulsionNodePseudoClassProperty = new NodePseudoClassProperty(propulsionIcon);
    NodePseudoClassProperty doorDerailmentNodePseudoClassProperty = new NodePseudoClassProperty(
        doorDetrainmentIcon);

    ObjectPropertyBase<CarComponentStatesDO> propulsionState = carComponentStatusModel
        .propulsionStatusProperty(carIndex);
    propulsionState.addListener(observable -> propulsionNodePseudoClassProperty
        .set(propulsionState.get().getComponentState().toLowerCase()));
    propulsionNodePseudoClassProperty.set(propulsionState.get().getComponentState().toLowerCase());

    ObjectPropertyBase<VehicleDoorStatesDO> doorDetrainmentState = carComponentStatusModel
        .detrainmentDoorStatusProperty(carIndex);
    doorDetrainmentState.addListener(observable -> doorDerailmentNodePseudoClassProperty
        .set(doorDetrainmentState.get().getDoorState().toString().toLowerCase()));
    doorDerailmentNodePseudoClassProperty
        .set(doorDetrainmentState.get().getDoorState().toString().toLowerCase());
    initUI();

    propulsionIcon.setVisible(!(footerPanelModel.getCurrentScreenId()
        .get() == (ScreenId.DOOR_SYSTEM_SCREEN.getId())
