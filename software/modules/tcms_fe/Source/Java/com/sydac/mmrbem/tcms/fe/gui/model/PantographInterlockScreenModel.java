...
import com.sydac.mmrbem.tcms.common.proto.TCMSProto.PantographInterlockData.PantographInterlockStatus;
...
public class PantographInterlockScreenModel extends AbstractScreenModel
{
  private ListProperty<CarComponentObjectProperty> acpmState = new SimpleListProperty<>(
      FXCollections.observableArrayList());

  private ListProperty<CarComponentObjectProperty> ocpmState = new SimpleListProperty<>(
      FXCollections.observableArrayList());

  public PantographInterlockScreenModel(int carIndex)
  {
    super(carIndex);
  }
  ...
  public void setACPMState(byte[] acpmStateList)
  {
    if (acpmStateList.length != 0)
    {
      try
      {
        TCMSProto.PantographInterlockData status = TCMSProto.PantographInterlockData.parseFrom(acpmStateList);
        for (PantographInterlockStatus vehicleState : status.getPantographInterlockStatusList())
        {
          updateCarComponentStatus(vehicleState.getAcpmState().name().toLowerCase(),
              vehicleState.getCarIndex(), acpmState);
          updateCarComponentStatus(vehicleState.getOcpmState().name().toLowerCase(),
              vehicleState.getCarIndex(), ocpmState);
        }
      }
   ...
   */
  public final ObjectPropertyBase<CarComponentStatesDO> acpmStatusProperty(int carIndex)
  {
    return getCarComponentPropertyFor(carIndex, acpmState);
  }
   ...
   * @return the ObjectPropertyBase of {@link CarComponentStatesDO} type
   */
  public final ObjectPropertyBase<CarComponentStatesDO> ocpmStatusProperty(int carIndex)
  {
    return getCarComponentPropertyFor(carIndex, ocpmState);
  }
