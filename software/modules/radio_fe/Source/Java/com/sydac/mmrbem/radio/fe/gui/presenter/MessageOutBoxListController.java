...
import static com.sydac.project.runtimedata.ProjectObjectConstants.RadioInputs.OUTBOX_MSG_LIST_SELECTED_ID;
...
public class MessageOutBoxListController extends AbstractRadioController
{
  @FXML
  ...
  private ChangeListener<Number> selectedOutgoingMessageIdListener;
  ...
  {
    messageScreenModel.getOutBoxMessageList().removeListener(messageListListener);
    messageScreenModel.getSelectedOutBoxMessageId().removeListener(selectedMessageIdListener);
    messageScreenModel.getSelectedOutBoxMessageId().removeListener(selectedOutgoingMessageIdListener);
  }
  ...
  public void initializeImpl()
  {
    ...
    setTableProperties(messageScreenModel.getOutBoxMessageList());

    messageListListener = ((observable, oldVal,
        newVal) -> updateScreenSelectedMessageCode(messageScreenModel.getSelectedOutBoxMessageId().get()));
    messageScreenModel.getOutBoxMessageList().addListener(messageListListener);

    selectedMessageIdListener = ((observable, oldVal,
        newVal) -> updateScreenSelectedMessageCode(messageScreenModel.getSelectedOutBoxMessageId().get()));
    messageScreenModel.getSelectedOutBoxMessageId().addListener(selectedMessageIdListener);

    selectedOutgoingMessageIdListener = ((observable, oldVal,
        newVal) -> updateScreenSelectedMessageCode(messageScreenModel.getSelectedOutBoxMessageId().get()));
    messageScreenModel.getSelectedOutBoxMessageId().addListener(selectedOutgoingMessageIdListener);
  }

  private void updateScreenSelectedMessageCode(Number newVal)
  {

    MessageTableDO message = messageScreenModel.getOutBoxMessageList().stream()
        .filter(s -> s.getMsgId() == newVal.intValue()).findFirst().orElse(null);

    if (message != null)
    {

      RadioFactory.getServiceRequestHandler(scenarioCarIndex).sendRequest(OUTBOX_MSG_LIST_SELECTED_ID,
          message.getMsgId());
  ...
  private void highLightSelectedMessage(MessageTableDO message)
  {
    ...
    int index = message == null ? -1 : messageScreenModel.getOutBoxMessageList().indexOf(message);
  ...
  private class RowChangeHandler implements ChangeListener<Object>
  {
    ...
    public void changed(ObservableValue<?> ov, Object oldVal, Object newVal)
    {
      ...
      messageScreenModel.setSelectedOutBoxMessageId(messageList.get((int)newVal).getMsgId());
