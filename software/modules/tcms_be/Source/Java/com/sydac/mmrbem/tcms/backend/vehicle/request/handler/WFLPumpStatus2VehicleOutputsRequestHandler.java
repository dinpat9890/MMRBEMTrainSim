...
/**
 * This class handles WFL Pump Status 2 update from vehicle outputs
 * 
 */
public class WFLPumpStatus2VehicleOutputsRequestHandler implements
    ITCMSVehicleOutputsRequestHandler<TCMSOutputsWFLSystemScreenImpl<Integer, IAppOutputsWFLSystemScreenHandler>>
{

  protected final List<Entry<VehicleWFLPumpStatusDO>> wflPumpStatus2 = new ArrayList<>();
      ...
      TCMSOutputsWFLSystemScreenImpl<Integer, IAppOutputsWFLSystemScreenHandler> request)
  {
    try
    {
      VehicleWFLPumpStatusDO vehicleWFLPumpStatusDO = ITCMSVehicleOutputsRequestHandler
          .getVehicleStatusDo(wflPumpStatus2, request.getCarIndex(), VehicleWFLPumpStatusDO.class);
      setStatus(request.getPropertyKey(), request.getNewValue(), vehicleWFLPumpStatusDO);
      ITCMSVehicleOutputsRequestHandler.updateVehicleStatusDOList(wflPumpStatus2, vehicleWFLPumpStatusDO,
          request.getCarIndex());
      setWFLPumpStatus(wflPumpStatus2, request.getVehicleDetailModelList(),
          builder -> request.getHandler().setWflPumpStatus2(builder.build().toByteArray()));
    }
  ...
  {
    if (propertyKey.equals(ITCMSVehicleOutputsWFLSystemProperties.WFL_PUMP_2))
    {
